(function () {
    ...
    function quizFactory($http) {
        ...
        function getQuestions(quizName) {
            return $http.get('/simple-quiz/api/quizzes/' + quizName).then(function (response) {
        ...
        function sendAnswers(quizName) {
            return $http.post('/simple-quiz/api/quizzes/' + quizName, answers).then(function (response) {
                return response.data;
            });
        }

        function sendFeedback(email, quiz, rating, feedback) {
            return $http.post('/simple-quiz/api/quizzes/feedback/' + quiz, {
                email: email,
                quiz: quiz,
                timestamp: Date.now(),
                rating: rating,
